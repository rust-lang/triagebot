...
static LINKED_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(" (?P<issue>#[0-9]+)(?P<end>[^a-zA-Z0-9])").unwrap());
...
fn fix_linked_issues<'a>(body: &'a str, full_repo_name: &str) -> Cow<'a, str> {
    let replace_by = format!(" {full_repo_name}${{issue}}${{end}}");
    parser::replace_all_and_ignores(&LINKED_RE, body, replace_by).into()
}
