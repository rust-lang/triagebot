...
*Please move them to the PR description, so they don't spam the issues with references to the commit and so bot can canonicalize them to avoid issues with subtree.*
{issue_links_commits}",
        ))
    }
}
