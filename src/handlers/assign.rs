        ...
        AssignInput::ReadyForReview => {
                ...
                && assign_command.is_none()
            {
                    ...
                    })
                {
                    ...
                    update_and_has_enough_approvals(ctx, &community_reviews, &event.issue).await?
                }
            } else {
...
/// Determine if the issue has enough approval reviews
async fn has_enough_approvals(
    ctx: &Context,
    ...
    let approval_reviews: HashSet<_> = reviews
        .iter()
...
/// Determine if the issue has enough approval reviews and update the PR
/// label to refect it.
async fn update_and_has_enough_approvals(
    ctx: &Context,
    config: &AssignCommunityReviewsConfig,
    issue: &Issue,
) -> anyhow::Result<bool> {
    let enough_reviews = has_enough_approvals(ctx, config, issue).await?;
...
pub(super) async fn handle_comment(
    ...
    if event.issue.labels.contains(&github::Label {
        ...
        update_and_has_enough_approvals(ctx, min_reviews, &event.issue).await?;
    }
